...
import (
	...
	"github.com/maany-xyz/maany-app/app"
	"github.com/maany-xyz/maany-app/app/params"
)

// NewRootCmd creates a new root command for maanyappd. It is called once in the
// main function.
...
func tempDir() string {
    dir, err := os.MkdirTemp("", "maanyappd")
	if err != nil {
...
func setCustomEnvVariablesFromClientToml(ctx client.Context) {
	...
	// gas
    setEnvFromConfig("gas", "MAANYAPPD_GAS")
    setEnvFromConfig("gas-prices", "MAANYAPPD_GAS_PRICES")
    setEnvFromConfig("gas-adjustment", "MAANYAPPD_GAS_ADJUSTMENT")
	// fees
    setEnvFromConfig("fees", "MAANYAPPD_FEES")
    setEnvFromConfig("fee-account", "MAANYAPPD_FEE_ACCOUNT")
	// memo
    setEnvFromConfig("note", "MAANYAPPD_NOTE")
}
...
# bash example
echo '. <(maanyappd enable-cli-autocomplete bash)' >> ~/.bash_profile
source ~/.bash_profile

# zsh example
echo '. <(maanyappd enable-cli-autocomplete zsh)' >> ~/.zshrc
source ~/.zshrc
`,
