...
# Derive version from git if available; fall back to dev string
VERSION := $(shell (git describe --tags --dirty --always 2>/dev/null || echo 0.0.0-dev) | sed 's/^v//')
...
ifeq ($(OS),Windows_NT)
	...
	go $@ -mod=readonly $(BUILD_FLAGS) $(BUILD_ARGS) ./cmd/maanyappd
endif
...
build-static-linux-amd64: go.sum $(BUILDDIR)/
	...
	$(DOCKER) cp neutronbinary:/bin/maanyappd $(BUILDDIR)/maanyappd-linux-amd64
...
install-test-binary: check_version go.sum
	go install -mod=readonly $(BUILD_FLAGS_TEST_BINARY) ./cmd/maanyappd
...
draw-deps:
	...
	@goviz -i ./cmd/maanyappd -d 2 | dot -Tpng -o dependency-graph.png
...
start: kill-dev install-test-binary
	@echo "Starting up maanyappd alone..."
	export BINARY=maanyappd CHAINID=test-1 P2PPORT=26656 RPCPORT=26657 RESTPORT=1317 ROSETTA=8080 GRPCPORT=8090 GRPCWEB=8091 STAKEDENOM=uapp && \
...
kill-dev:
	@echo "Killing maanyappd and removing previous data"
	-@rm -rf ./data
	-@killall maanyappd 2>/dev/null
...
build-docker-image:
	...
	@docker buildx build --load --build-context app=. -t neutron-node --build-arg BINARY=maanyappd .
