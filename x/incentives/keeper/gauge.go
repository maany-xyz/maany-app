...
import (
    ...
    epochtypes "github.com/maany-xyz/maany-app/x/epochs/types"
...
func (k Keeper) CreateGauge(ctx sdk.Context, isPerpetual bool, owner sdk.AccAddress, coins sdk.Coins, distrTo lockuptypes.QueryCondition, startTime time.Time, numEpochsPaidOver uint64, poolId uint64) (uint64, error) {
    ...
    // Only support ByDuration lock query type. NoLock and ByGroup are unsupported.
    if distrTo.LockQueryType != lockuptypes.ByDuration {
        return 0, fmt.Errorf("unsupported gauge type: only ByDuration is allowed")
    }
    ...
    // For ByDuration gauges, pool id must be 0.
    if poolId != 0 {
        return 0, fmt.Errorf("pool id must be 0 for ByDuration gauges (pools disabled)")
    }
	...
	// https://github.com/maany-xyz/maany-app/issues/6580
...
func (k Keeper) checkIfDenomsAreDistributable(ctx sdk.Context, coins sdk.Coins) error {
    // Pools are disabled; allow any denom (no protorev checks).
    return nil
}
