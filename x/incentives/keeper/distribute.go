...
import (
	...
	"github.com/maany-xyz/maany-app/osmoutils/coinutil"
...
func (k Keeper) AllocateAcrossGauges(ctx sdk.Context, activeGroups []types.Group) error {
	for _, group := range activeGroups {
		...
		// https://github.com/maany-xyz/maany-app/issues/6556
		...
		for gaugeIndex, distrRecord := range group.InternalGaugeInfo.GaugeRecords {
			...
			// https://github.com/maany-xyz/maany-app/issues/6558
...
func (k Keeper) FilteredLocksDistributionEst(ctx sdk.Context, gauge types.Gauge, filteredLocks []lockuptypes.PeriodLock) (types.Gauge, sdk.Coins, bool, error) {
    // Convert QueryCondition from generated (v29) type to local lockuptypes for keeper interface.
    qc := lockuptypes.QueryCondition{
        LockQueryType: lockuptypes.LockQueryType(gauge.DistributeTo.LockQueryType),
        Denom:         gauge.DistributeTo.Denom,
        Duration:      gauge.DistributeTo.Duration,
        Timestamp:     gauge.DistributeTo.Timestamp,
    }
    TotalAmtLocked := k.lk.GetPeriodLocksAccumulation(ctx, qc)
...
func (k Keeper) doDistributionSends(ctx sdk.Context, distrs *distributionInfo) error {
    ...
    for id := 0; id < numIDs; id++ {
        to := distrs.idToDecodedRewardReceiverAddr[id]
        amt := distrs.idToDistrCoins[id]
        if !amt.Empty() {
            if err := k.bk.SendCoinsFromModuleToAccount(ctx, types.ModuleName, to, amt); err != nil {
                return err
            }
        }
...
func (k Keeper) syncGroupWeights(ctx sdk.Context, group types.Group) error {
    // Pools are disabled; leave group weights unchanged, simply persist current state.
...
func (k Keeper) calculateGroupWeights(ctx sdk.Context, group types.Group) (types.Group, error) {
    // Pools are disabled; do not recalculate based on pool volumes.
    // Keep existing weights and recompute total weight as sum of current weights.
    total := zeroInt
    updated := types.Group{
    ...
    for i, r := range group.InternalGaugeInfo.GaugeRecords {
        updated.InternalGaugeInfo.GaugeRecords[i] = r
        total = total.Add(r.CurrentWeight)
    }
    updated.InternalGaugeInfo.TotalWeight = total
    return updated, nil
}
...
func (k Keeper) syncVolumeSplitGroup(ctx sdk.Context, group types.Group) error {
    // Pools are disabled; weights are not updated dynamically.
    updatedGroup, _ := k.calculateGroupWeights(ctx, group)
...
// NoLock gauge uptime helper removed (pools disabled).
...
func (k Keeper) distributeInternal(
    ...
    if gauge.DistributeTo.LockQueryType == lockuptypes.NoLock {
        // Pools are disabled; skip NoLock gauges entirely.
		...
		for _, lock := range locks {
			...
			for _, coin := range remainCoins {
				...
				if coin.Denom == minValueForDistr.Denom {
                    ...
                    // Pools are disabled; do not attempt cross-denom valuation.
                    // Only enforce threshold when denom matches the target above.
                }
