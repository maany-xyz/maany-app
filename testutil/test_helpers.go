...
import (
	...
	"github.com/maany-xyz/maany-app/utils"
	...
	ictxstypes "github.com/maany-xyz/maany-app/x/interchaintxs/types"
...
func (suite *IBCConnectionTestSuite) FundAcc(acc sdk.AccAddress, amounts sdk.Coins) {
    ...
    // Mint using the transfer module account, which has minter permission.
    err := bankKeeper.MintCoins(suite.ChainA.GetContext(), types.ModuleName, amounts)
    suite.Require().NoError(err)

    err = bankKeeper.SendCoinsFromModuleToAccount(suite.ChainA.GetContext(), types.ModuleName, acc, amounts)
    suite.Require().NoError(err)
}
...
func (suite *IBCConnectionTestSuite) FundModuleAcc(moduleName string, amounts sdk.Coins) {
    ...
    err = bankKeeper.SendCoinsFromModuleToModule(suite.ChainA.GetContext(), types.ModuleName, moduleName, amounts)
    suite.Require().NoError(err)
}
